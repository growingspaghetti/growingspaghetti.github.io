...
```rust
fn median<'a>(vec: &Vec<&'a str>, a: usize, c: usize) -> &'a str {
    let b = (c - a) / 2;
...
fn sink_fully(vec: &Vec<&str>, depth: &mut usize, blocker: &str) {
    while vec[*depth] < blocker {
        *depth += 1;
    }
}

fn float_fully(vec: &Vec<&str>, depth: &mut usize, blocker: &str) {
    while vec[*depth] > blocker {
        *depth -= 1;
    }
}

fn break_through(vec: &mut Vec<&str>, sinker_depth: &mut usize, float_depth: &mut usize) {
    vec.swap(*sinker_depth, *float_depth);
    *sinker_depth += 1;
    *float_depth -= 1;
}

fn release(vec: &mut Vec<&str>, sinker0: usize, float0: usize) {
